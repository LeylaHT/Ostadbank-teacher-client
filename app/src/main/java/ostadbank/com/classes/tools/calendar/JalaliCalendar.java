package ostadbank.com.classes.tools.calendar;
...
public class JalaliCalendar extends Calendar {
  ...
  //  private static final long ONE_DAY = 24 * ONE_HOUR;
    ...
    public JalaliCalendar(TimeZone zone, Locale aLocale) {
        ...
        YearMonthDay yearMonthDate = new YearMonthDay(calendar.get(YEAR),
    ...
    public Date getGregorianDate(String jalaliDate) {
        ...
        YearMonthDay jalali = new YearMonthDay(Integer.parseInt(arr[0]),
                Integer.parseInt(arr[1]) - 1, Integer.parseInt(arr[2]));

        YearMonthDay gDate = jalaliToGregorian(jalali);
    ...
    public String getJalaliDate(Date gDate) {
       ...
       // Date dt = gCal.getTime();
        ...
        YearMonthDay gMyDate = new YearMonthDay(myYear, myMonth, myDay);

        YearMonthDay jDate = gregorianToJalali(gMyDate);

        String jalali = jDate.year + "/" + (jDate.month + 1) + "/" + jDate.day;
    ...
    JalaliCalendar(int year, int month, int dayOfMonth, int hourOfDay,
        ...
        YearMonthDay yearMonthDate = jalaliToGregorian(new YearMonthDay(
                fields[1], fields[2], fields[5]));
        cal = new GregorianCalendar(yearMonthDate.getYear(),
                yearMonthDate.getMonth(), yearMonthDate.getDay(), hourOfDay,
                minute, second);
    ...
    public static int weekOfYear(int dayOfYear, int year) {
        switch (dayOfWeek(JalaliCalendar.jalaliToGregorian(new YearMonthDay(
                year, 0, 1)))) {
        case 2:
            dayOfYear++;
            break;
    ...
    public static int dayOfWeek(YearMonthDay yearMonthDate) {

        Calendar cal = new GregorianCalendar(yearMonthDate.getYear(),
                yearMonthDate.getMonth(), yearMonthDate.getDay());
    ...
    protected void computeTime() {
        ...
        if (!isTimeSet && !isTimeSeted) {
            ...
            cal.set(yearMonthDate.getYear(), yearMonthDate.getMonth(),
                    yearMonthDate.getDay(), internalGet(HOUR_OF_DAY),
        ...
        case DST_OFFSET: {
            if (isSet(YEAR) && isSet(MONTH) && isSet(DATE) && isSet(HOUR)
                ...
                cal.set(yearMonthDate.getYear(), yearMonthDate.getMonth(),
                        ...
                        internalGet(MINUTE), internalGet(SECOND));
                cal.set(field, value);
                yearMonthDate = gregorianToJalali(new YearMonthDay(
    ...
    protected void computeFields() {
        ...
        if (!areFieldsSet) {
            ...
            super.set(DAY_OF_WEEK,
                    dayOfWeek(jalaliToGregorian(new YearMonthDay(fields[1],
                            fields[2], fields[5]))));
            // ***
    ...
    public void add(int field, int amount) {
        ...
        if (field == MONTH) {
            ...
            gc.add(field, amount);
            yearMonthDate = gregorianToJalali(new YearMonthDay(gc.get(YEAR),
